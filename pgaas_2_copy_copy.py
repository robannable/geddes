...
class OllamaAPIHandler:
    def __init__(self, base_url="http://localhost:11434", max_retries=3, timeout=30):
        ...
        for attempt in range(self.max_retries):
            try:
                response = self.session.post(
                    f"{self.base_url}/api/generate",  # Changed from /api/chat to /api/generate
                    headers=headers,
                    json=data,
                ...
                return {"message": {"content": response.json().get("response", "")}}
                
            except requests.exceptions.ConnectionError:
                if attempt == self.max_retries - 1:
                    raise ConnectionError("Failed to establish connection with Ollama server")
...
def get_about_info():
    ...
    result = ErrorHandler.handle_file_operation(
        lambda: open(about_file_path, 'r').read().strip()
    )
...
class ResponseProcessor:
    @staticmethod
    def process_perplexity_response(response_json, user_name, prompt, unique_files, chunk_info):
...
def get_perplexity_response(user_name, prompt):
    try:
        ...
        # Extract unique filenames and chunk info
        ...
        api_handler = OllamaAPIHandler(st.secrets['PERPLEXITY_API_KEY'])
        ...
        # Get list of recent history files and their contents
        history_dir = os.path.join(script_dir, "history")
        ...
        # Fix the history processing
        if isinstance(today_history, list):
            for entry in today_history:
                if isinstance(entry, dict):  # Make sure we only process dictionary entries
        ...
        # Create temporal context with explicit conversation history
        time_context = f"""
        ...
        return ResponseProcessor.process_perplexity_response(
            response_json, 
            user_name, 
            prompt, 
            unique_files, 
