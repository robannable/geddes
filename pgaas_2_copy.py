...
def get_perplexity_response(user_name, prompt):
    try:
        ...
        history_dir = os.path.join(script_dir, "history")
        history_files = sorted([f for f in os.listdir(history_dir) if f.endswith('_conversation_history.md')], reverse=True)[:7]  # Last 7 days
        
        # Create temporal context
        current_date = datetime.now()
        time_context = f"""
        Current date and time: {current_date.strftime('%d-%m-%Y %H:%M')}
        
        Recent conversation history available for these dates:
        {', '.join(f.split('_')[0] for f in history_files)}
        
        When discussing time-related queries:
        - Today refers to {current_date.strftime('%d-%m-%Y')}
        - This week includes conversations from {history_files[-1].split('_')[0]} to {history_files[0].split('_')[0]}
        - Please reference timestamps in the conversation history accurately
        """
