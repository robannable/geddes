...
def get_perplexity_response(prompt, api_key):
    ...
    documents = [chunk for chunk, _ in document_chunks_with_filenames]
...
if st.button('Show Chat History'):
    ...
    for entry in history:
        ...
        <span style="color: #1e90ff; font-weight: bold;">{entry['name']} - {entry['date']} {entry['time']}</span><br>
        <span style="color: #32cd32; font-style: italic;">{entry['question']}</span>
        </div>
        """, unsafe_allow_html=True)
        
        st.markdown(f"**Patrick Geddes:** {entry['response']}")
        st.markdown(f"**Sources:** {entry['unique_files']}")
        st.markdown(f"**Chunks used:** {entry['chunk_info']}")
        st.markdown("---")
